...
prompt="""
There are several numbers now, and we want to achieve 24 points (by randomly combining them through four arithmetic operations to reach 24, each number can only be used once, all numbers must be used, and only the numbers in Input can be used):
if there are only 1 number "24" in the input sequence,the task is successfully achieved.
...
Contnet:
Input:[22]
Output:Stop,can not achieve the goal
}}
{{Operation:(1)
Contnet:
Input:[24]
Output:Successfully achieved the goal
}}
When selecting numbers, please check whether both chosen numbers are present in the input.
only choose one operation in one step
"""
...
just give one step plan.each answer should be in "{{}}".Output according to the format in the system prompt.The number of answer is {input1}!!!
Example:
{{Operation:(2)
Content:answer1}},
{{Operation:(2)
...
"""
value_prompt2="""
The current goal is to obtain the unique number 24 by performing addition, subtraction, multiplication, and division on the input sequence of numbers.
Currently, a step plan has been generated, and now we need to evaluate its effectiveness. We need to determine if the remaining numbers can reach 24.(You can only use the numbers that appear in the input, and each number can be used only once.)
...
class SimpleEnv:
    ...
    def step(self, action_lst,mu):
        ...
        while(redo<2):
            ...
            if self.steps<=10:
                pro.append({"role": "user", "content":prompt1.format(input=curr,input1=8)})
            else:
